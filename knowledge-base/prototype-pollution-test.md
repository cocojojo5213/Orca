> The "PP to Template RCE" quick-hits row lives in the second half of this file, under the section heading `Source topic: prototype-pollution-advanced` — find it by searching that heading. The primary hunting ground is Node deep merge; other stacks are still worth a single probe if their JSON path also merges — drop it when no gadget lines up; it is not off-limits.

# prototype-pollution


# Source topic: prototype-pollution
...
# Source topic: prototype-pollution-advanced

# Prototype Pollution Advanced — RCE & Gadget Exploitation

### PP to Template RCE (quick-hits pointer)

Recognize: Node / Express; JSON body or query can write `__proto__` / `constructor.prototype` (first send `{"__proto__":{"pptest123":"1"}}` and confirm the side effect still shows on a clean request); downstream there is `res.render`, EJS, Pug, Handlebars, or `child_process.spawn`.

Attack:

1. Only go for the gadget once pollution is confirmed; do not start by spraying RCE payloads.
2. EJS: inject `outputFunctionName`; Pug: `block.type` + `val`; if `__proto__` is blocked, switch to `constructor.prototype`.
3. Without a template engine, try `shell` + `NODE_OPTIONS`. Use `echo MARKER && id` for the command — no reverse shells.

Counts as: after the next template render or spawn, the chosen marker or `uid=` shows up in the response/logs. Pollution succeeds but the command never runs → not yet a finding.

False positives: only front-end display changes; no template/spawn gadget; the pollution dies after a single request and the next render does not pick it up. One site not hitting is no reason to delete the quick-hits row. On Node deep merge, open the gadget chain first; other stacks can be probed if the merge accepts the keys — drop it when nothing lines up.

Gadget table and payloads: see the English sections below, plus the KNOWN_GADGETS appendix.
