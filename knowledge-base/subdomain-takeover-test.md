...
(The CORS-bypass note from earlier drafts is retained only for understanding the impact chain; **for SRC, do not hunt or write up CORS on its own**.)
...
└── Post-takeover impact assessment
    ...
    ├── (Do not report CORS standalone.) Subdomain XSS / sensitive read/write after takeover → report as a main-business vulnerability
