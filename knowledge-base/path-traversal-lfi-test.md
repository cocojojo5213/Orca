> The shortlist entry "Nginx alias missing slash" lives under §22; just search by heading. If the site already serves `/static` `/assets` `/img`, you can hit `/static../` without first seeing the misconfigured alias. Without such prefixes, use the ordinary `file=` traversal — do not fire empty shots just to tick the shortlist.
...
### Nginx alias missing slash (referenced in the shortlist)

Recognize: the site already serves prefixes like `/static`, `/assets`, `/img`, `/files` (black-box you cannot see the nginx config — if the prefix exists, fire); or a Caddy template consumes user input.

Fire:

1. `GET /static../etc/passwd`, `GET /static../etc/nginx/nginx.conf`, `GET /assets..%2f..%2fetc%2fpasswd`.
2. Cross-check: `/static/../etc/passwd` is usually normalized away; **the missing-slash shot** is the one that hits the alias prefix replacement.
3. Caddy template surface: try `{{readFile "/etc/passwd"}}`, `{{.Req.Host}}` in the input. Only a successful file read counts; merely reflecting Host does not count as this item.

Counts as: reading config / secrets / `nginx.conf` outside the web root. Only being able to list css/images that already exist in the static directory → not a win.

False positives: 404; both sides of the alias have slashes so normalization cannot escape; the Caddy template does not execute user input. If a single site misses, do not delete this shortlist row. If a static prefix exists, fire — you don't need to see the nginx config first. Ordinary `file=` traversal still follows the general section above; the two shots do not replace each other.
...
### Repo root served as static (koa-static / express.static pointing to cwd)

Recognize: the page or `/package.json` lets you GET Node project files directly (`package.json`, `routes/`, `index.js`), meaning the static middleware points at the repo root, not `public/`.

Fire: no login needed. Follow node-config environment names and hit `/config/online.yml` `/config/default.yml` `/config/production.yml` `/config/pre.yml` `/config/dev.yml`, plus `.env`. No need to start with `../`.

Counts as: the yml/json contains **complete DB credentials or real cloud secret values**. Only README / keyless source / an empty template with just ports → not a win (info-level is not reported).

False positives: the static root only serves `public`; `package.json` returns 200 but config returns 404; the yml has no credentials. If a single site misses, do not delete this recognition method.

### .NET publish artifacts on a static bucket (aspx still GET-able)

Recognize: old .NET publish artifacts sit on an object store / static bucket (MSS `product-fe-*` style). aspx source is still GET-able — it isn't served only after compilation. The same-directory `web.config` is often blocked by a WAF. This is not the Node repo root (that shot is in the previous section).

Fire: no login needed. If `web.config` returns 456, don't stop — hit the same-directory `App.config`, `Web.Release.config`, `*.exe.config`, `bin/*.dll.config`. If there's a bin, also sweep `*.pdb` to see whether anything is tied to config.

Counts as: full credentials or a DeveloperToken, not `ENTER_YOUR_*` placeholders.

False positives: aspx returns 200 but config returns 404; only source code with no keys; placeholder passwords. Real secret values are never committed to the repo. If a single site misses, do not delete this recognition method.

### Reading process environment on public VS Code-family

Recognize: public code-server; opening `/login` drops straight into the editor (no login wall).

Fire: no login — `GET /remote-resource?path=/proc/self/status` to read Uid, then `path=/proc/self/environ`. Public VS Code-family, `AuthType.None` in the page/`_static` is a common skin; even without that string, still fire. Do not stop at the README. If environ holds an SSH private key (often Base64 PEM): **after decoding, compare the fake key** against the Git host in that environment — a real key answers with the Git username, a fake key gives Permission denied. Merely copying the string is not confirming a key.

Counts as: environ contains **someone else's** email and a full SSH private key, and the real key can reveal the Git username. Only install scripts / public templates → not a win.

False positives: `path` requires login; only the README is readable; real and fake keys give the same Permission denied. The auth enum name not being `None` is not a false positive. If a single site misses, do not delete this shortlist row. Real secret values are never committed to the repo.
