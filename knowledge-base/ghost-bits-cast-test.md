...
Non-email CRLF: hit it on-site per protocol (`crlf-injection-test.md` is collapsed to one line).
...
For any missing ASCII byte, compute it yourself with `chr((k<<8)|T)` (§1 / §3); the full byte-by-byte double CJK tables were cut.
