...
> **Recon pacing follows only** `../rules/engagement-guide.md` §1.0.1 / §2.1: search only the **current single** seed; no new searches until this seed's remaining live surface is exhausted. Once you recognize a quick-hits shape, only attack the current site; scanning the whole internet with that shape is forbidden. Quality root domains only flow back into the queue; search again only after this seed is done.
>
> **A full nuclei sweep doesn't count as progress.** nuclei is only an aid when you need known CVEs / exposed surfaces (actuator, swagger, known middleware); running "every template once" is not this site's matrix.
>
> Asset search: this machine has **no FOFA MCP**. Use opencode's `websearch` tool to search by brand/domain/asset leads (domains from official sites, subdomains, ICP filings, open-source repos, press releases), then `curl "https://crt.sh/?q=%.<root-domain>&output=json"` for passive subdomain enumeration; also collect hosts from the target's own JS/API/mobile bundles. A single query serves only the current seed. **Forbidden** to write email / keys into this file or the conversation.

### Asset-Search Minimal Cheat Sheet (no FOFA MCP on this machine; not an opening ritual)

```
websearch query terms:
  brand/group name + official site / ICP filing / subsidiaries
  search the root domain directly → collect subdomain/platform leads (domains in press releases, docs sites, open platforms)
  after entry: keep collecting hosts from JS / APIs / mobile bundles

Passive subdomain enumeration:
  curl -s "https://crt.sh/?q=%.<root-domain>&output=json" | jq -r '.[].name_value' | sort -u

Alive filter: curl -sI and check 200/301/403; only live surfaces go into the queue
```

Quake / Fengniao only fill gaps for the **current seed**; they're not a mandatory opening run. Syntax references and ROI filtering are in the sections below; filtering still follows `../rules/engagement-guide.md` (drop junk / drop non-live / shareholding gate).
...
> **This section does not dictate pacing.** For free-jumping / one-seed / where to spend effort first, follow `../rules/engagement-guide.md` + `../rules/what-to-hunt.md` §1.1. This section is only for: how to think about parameters, and not missing error pages / old versions / mobile APIs. The commands and ideas still hold.
...
> **This section does not dictate pacing.** For jumping seeds / switching sites, follow `../rules/engagement-guide.md`; for where to spend effort first, follow `../rules/what-to-hunt.md` §1.1. The Priority below is not a second recon methodology, nor an SRC severity scale. The commands and parameter ideas still hold.
...
Running every template once doesn't count as progress. Narrow the templates only when you need known CVEs / exposed surfaces.

```bash
# Narrow: known CVEs / exposed surfaces; don't treat this as an opening full sweep
...
```
□ CORS: permanently skipped for SRC (don't hunt, don't write up; see `../rules/what-to-hunt.md`) — not a quick win
