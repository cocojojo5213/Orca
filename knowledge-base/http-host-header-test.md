> The shortlist entry "Host poisoning of reset links" is under §2 — search by that heading. If there is a reset/activation endpoint, attack with Host / XFH (`../rules/engagement-guide.md` §4.2.2) — no need to see what the email looks like first. If no such endpoint exists, do not modify Host on random pages just to tick the shortlist.
...
### Host Poisoning of Reset Links (Shortlist Pointer)

Confirm: the asset list has a reset / activation / invitation email endpoint (`../rules/engagement-guide.md` §4.2.2). No need to first see whether the email is assembled from Host.

Attack:

1. Run a reset against your own account and see whether the link in the email uses the Host from the request.
2. Run it again, changing `Host` to a collaborator domain. If blocked with 400/403, **keep the original site in Host** and only add `X-Forwarded-Host: <collaborator-domain>` (many frameworks validate only Host yet assemble the link from XFH).
3. Also try `Host: <original-site>.<collaborator-domain>`, `Host: <original-site>:@<collaborator-domain>`, and an absolute URI in the request line.

Counts as success: the collaborator domain receives a reset/activation link carrying a token, and that token can be used on the **original site** to change someone else's password. Merely reflecting Host while the email does not follow → not success.

False positives: the email still shows the original site; the token is bound to the current browser session and won't open; only the display domain changed and the link still needs the original site's verification code. A single-site miss does not remove this shortlist row. If a reset endpoint exists, still attack with Host/XFH — no need to see the email first. Only when no such endpoint exists do you skip modifying Host on random pages.

OAuth variant: an unauthenticated authorization-address endpoint (of the `GetAuthorizationUrl` family) assembles `redirect_uri` from Host in its response. After changing Host, the official identity page still returns 200 and the callback in the URL is an external domain → success (the code lands on the external domain). Identity page rejects the external domain → false positive. Also try XFH in case the site itself ignores it.
