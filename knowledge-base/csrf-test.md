...
> **SRC discipline:** In this doc, a **successful CSRF write** and the like can still be reported as medium/high severity (only when it is a cross-user or sensitive write).  
> CORS mentioned here is only to build the chain-of-understanding; it is **forbidden** to submit a standalone CORS report as the primary finding (`../rules/what-to-hunt.md`: not hunted, not reported).
...
□ JSON CSRF: prove by whether the write operation succeeds cross-site; do not submit a standalone CORS report (SRC: not hunted, not reported)
□ Check OAuth flows for missing state parameter
□ QR / scan-to-login: whether the token can be placed in the URL, and whether an already-logged-in user opening it equals confirmation (§18)
...
> **SRC:** This section only helps understand "read the token → then CSRF write".  
> The primary finding is **CSRF / privileged write**; do **not** file a separate CORS report.
...
## 18. SCAN-TO-LOGIN CSRF → ACCOUNT TAKEOVER (Shortlist Pointer)

> Essence: an already-logged-in user is tricked into opening a URL; their cookies travel along automatically, which equals clicking "confirm scan" on the attacker's behalf.  
> Same class as password-change / transfer CSRF; the hit lands on that "confirm scan" step. The shortlist row points to this section.  
> What enters the knowledge base is the technique, not any site's actual token value.

### 18.1 What to Recognize (Recognizable on Arrival)

Fire these as opening shots when all appear together:

- The login page has a QR code / "scan with App"
- Without logging in, calling the token-generation endpoint returns a `token` (or ticket / scene / sid) in the response, often with an expiry time
- The same site also has a login URL that consumes query parameters: `/login?token=`, `?ticket=`, `agentType=` and the like

### 18.2 How the Chain Runs (Two Browsers)

```
Attacker (incognito, not logged in)
  → open the scan-to-login page
  → capture the request/response that generates the token (common paths contain qrcode / qr / gen_login_token / is_token_login)
  → put the token into the login page URL

Victim (already logged in on the same site)
  → open that URL (a top-level navigation is enough — matches SameSite=Lax top-level GET)
  → the server binds that token to the victim's account (equals confirming the scan)

Attacker incognito
  → after polling/refreshing, the session becomes the victim's account
```

Crafted shapes (change field names per the actual site; do not copy blindly):

```
https://<target-login-page>?token=<token-obtained-just-now>
https://<target-login-page>?agentType=<number>&token=...&lang=...
```

### 18.3 What Counts as Success

The attacker's **unauthenticated** side shows the victim's avatar/account and can act as the victim.  
Merely proving "a logged-in user opened the link" does not count; the login state must land on the incognito side.

### 18.4 False Positives (Shape Is Not a Bug, Not "This Site Was Not Broken Through")

- The token expires and clicking it logs in neither side
- Opening the URL only issues a new QR code and does not bind the token to the currently logged-in account
- A real mobile App scan is mandatory; opening the URL does not count as confirmation

If this site 401s or rotates the token without success → it only means this site does not work; **hit the same technique on the next site regardless**; do not remove this shortlist row.

### 18.5 Do Not Confuse With Neighboring Techniques

| This entry | Don't mistake it for |
|------|--------|
| CSRF on the "confirm scan" step | Directly reading business APIs without logging in |
| Victim must be logged in (needs a session) | Passwordless login bypass, entering admin with an empty password |
| A GET login URL is enough to confirm | CSRF that needs POST + custom headers to change state (that needs separate testing) |

OAuth missing `state` and handing the code to the victim (§9) is a close relative: both involve "a logged-in user opening a callback prepared by the attacker." The scan entry keys on the **QR token**, not an authorization code.

### 18.6 Opening Probe (Attack If There Is an Entry Point; Half-Minute Level)

1. Without logging in, open the scan-to-login page and capture the token-generation endpoint  
2. Check whether the response has a token / expiry field; check whether the front end assembles `/login?token=`  
3. From another profile where you are already logged in, open the assembled URL  
4. Return to the unauthenticated side and check whether it has become your account  

If step 4 holds → write an account-takeover report per `../rules/what-to-hunt.md`. Put only actual keys/token values in the report, not in the shortlist.

### 18.7 In-App Browsers of Social Apps (Attack Only When Seen)

When the page is opened in a social App's WebView and the App auto-attaches `pskey` / silent OAuth by domain: same-origin XSS can conveniently exfiltrate credentials, or click "confirm scan" on behalf of a logged-in user. **Not something to try on every site**; without this shell, still only attack the scan CSRF above. Not added to the shortlist.
